...
function setCookie(name, value) {
	...
	document.cookie = name + "=" + encodeURIComponent(value) + ";expires="
			+ exp.toUTCString();
	alert(name + "=" + encodeURIComponent(value) + ";expires="
			+ exp.toUTCString());
}
...
$(function() {
	...
	$("#changeCode").click(function() {
		$("#safeCode").attr("src", "validCode.do?id=" + Math.random());
	});
						...
						if (val == "添加") {
							...
							$("#span").append(
									'<br/><input name="address" type="radio" id="address0"/><span>'
											+ addr + '</span>');
							location.href = "Cart?action=addAddress&address="
									+ $("#addAddr").val();
						} else {
							var span = $("#span")
									.html(
	...
	$(".number").find("span").click(
			function() {
				...
				window.location = 'Cart?action=mod&entityId=' + data[0]
						+ '&quantity=' + number + '&index=' + data[1];
			});
	...
	$("#product .thumb img").mouseover(
			function() {
				...
				$("#product").append(
						'<img id="bigImg" src="files/' + name + '_big.jpg"/>');
	...
	$("#shoppingBag").mouseover(
			function() {
				...
				for (var i = 0; i < data.length; i++) {
					var d = data[i];
					html.push("<tr><td><img width='25' height='25' src='"
							+ d[0] + "'/></td><td>" + d[1] + "</td><td>" + d[2]
							+ "</td></tr>");
				}
