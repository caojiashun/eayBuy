...
<body>
	...
	<div id="main" class="wrap">
		...
		<div class="main">
			...
			<h2>修改分类</h2>
			<div class="manage">
				<form action="Category?action=update"
					method="post">
					<input type="hidden" name="id" value="${category.epcId}"/>
					<table class="form">
						<tr>
							...
							<td><select name="parentId">
									...
									<c:if test="${category.epcParentId!=0}">
										<c:forEach items="${categories}" var="item">
											<option value="<c:out value='${item.epcId}'/>"
												<c:if test="${item.epcId==category.epcParentId}">selected="selected"</c:if>><c:out
													value="${item.epcName}" /></option>
										</c:forEach>
									</c:if>
							</select></td>
						</tr>
