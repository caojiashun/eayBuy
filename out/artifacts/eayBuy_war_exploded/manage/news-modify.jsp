...
<body>
	...
	<div id="main" class="wrap">
		...
		<div class="main">
			<h2>修改新闻</h2>
			<div class="manage">
				<form action="News?action=update&entityId=${entityId}" method="post"
					id="newsAdd">
					<table class="form">
						<tr>
							...
							<td><input type="text" class="text" name="title"
								value="${title}" /> <c:if
									test="${not empty errors && not empty errors['title']}">
									<span class="error">${errors['title']}</span>
								</c:if> <c:if test="${empty errors || empty errors['title']}">
									<span></span>
								</c:if></td>
						</tr>
						<tr>
							...
							<td><textarea name="content" class='text'>${content}</textarea>
								...
								</c:if> <c:if test="${empty errors || empty errors['content']}">
									<span></span>
								</c:if></td>
						</tr>
						<tr>
